extends SceneTree
## Regressions found while completing DRIVE, including real replay and handoff.
...
func run() -> void:
	...
	# Reproducible rapid input across all profiles must stay within valid bounds.
	var random := RandomNumberGenerator.new()
	random.seed = 7307
	for index in range(10000):
		if index % 2000 == 0:
			city._load_stage(index / 2000, true)
	...
	# Actual replay button should reset the trip without charging gas.
	...
	for failure in failures:
		push_error(failure)
	print("REGRESSION PASS — exits, contacts, 10000 random frames, replay, state, venue" if failures.is_empty() else "REGRESSION FAIL")
