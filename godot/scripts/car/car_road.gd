...
##
## The car stays near the bottom-center. The town scrolls and rotates around it,
## so LEFT and RIGHT are literal screen directions instead of abstract choices.
...
func _process(delta: float) -> void:
	_view_rotation = lerp_angle(_view_rotation, _target_view_rotation, minf(1.0, delta * 8.0))
	queue_redraw()
...
func _draw_street(a_world: Vector2, b_world: Vector2) -> void:
	...
	var asphalt := Color(0.235, 0.225, 0.205, 1.0)
	var center := Color(0.70, 0.60, 0.31, 0.42)

	draw_line(a, b, curb, 72.0, true)
	draw_line(a, b, asphalt, 61.0, true)
	...
	while cursor < distance:
		var s := a + direction * cursor
		var e := a + direction * minf(cursor + 17.0, distance)
		draw_line(s, e, center, 2.4, true)
		cursor += 48.0


func _draw_tree(world: Vector2) -> void:
	var p := _world_to_screen(world)
	draw_rect(Rect2(p.x - 3, p.y + 5, 6, 13), Color(0.28, 0.20, 0.13, 1.0))
	draw_circle(p + Vector2(3, 4), 15.0, Color(0.07, 0.10, 0.06, 0.35))
	draw_circle(p, 14.0, Color(0.16, 0.26, 0.13, 1.0))
	draw_circle(p + Vector2(-8, 2), 8.0, Color(0.19, 0.30, 0.15, 1.0))
...
func _draw_house(world: Vector2, index: int) -> void:
	var p := _world_to_screen(world)
	draw_rect(Rect2(p - Vector2(22, 14), Vector2(44, 30)), _house_color(index))
...
func _draw_shop(world: Vector2, index: int) -> void:
	var p := _world_to_screen(world)
	draw_rect(Rect2(p - Vector2(28, 16), Vector2(56, 32)), _house_color(index))
...
func _draw() -> void:
	...
	# Draw every real street in the little town.
	...
	# Placeholder illustrated town objects.
